...
class PerformancesCurrentDensity(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        outputs["fc_current_density"] = inputs["dc_current_out"] / inputs["effective_area"]

    def compute_partials(self, inputs, partials, discrete_inputs=None):
        number_of_points = self.options["number_of_points"]

        partials["fc_current_density", "dc_current_out"] = (
            np.ones(number_of_points) / inputs["effective_area"]
        )

        partials["fc_current_density", "effective_area",] = (
            -inputs["dc_current_out"] / inputs["effective_area"] ** 2
        )
